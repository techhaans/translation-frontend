...
const Labels = () => {
  ...
  const fetchLanguagesAndTranslations = async (customerUid) => {
    try {
      const response = await fetch(`http://13.232.167.179:8082/customer/languages/${customerUid}`);
      if (response.ok) {
  ...
  const fetchLabels = async (cuid) => {
    try {
      setLoading(true);
      const response = await fetch(`http://13.232.167.179:8082/labels/${cuid}`);
      if (response.ok) {
        const data = await response.json();
        setLabels(data);
      } else {
  ...
  const fetchTranslationsFromAPI = async (customerUid, langs) => {
    try {
      ...
      for (const lang of langs) {
        ...
        const response = await fetch(`http://13.232.167.179:8082/translations/${customerUid}/${langCode}`);
        if (response.ok) {
          const data = await response.json();
  ...
  const handleSave = async () => {
    for (const lang of languages) {
      ...
      try {
        const response = await fetch(`http://13.232.167.179:8082/language/update/${lang.languageCode}`, {
          method: 'PUT',
